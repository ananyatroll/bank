  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        ...
        _Tile(Icons.person_outline, 'Name', 'TeleBank User', onTap: () => _editField(context, 'Name', 'TeleBank User', (v) async {
          await _set('user_name', v);
        })),
        _Tile(Icons.account_balance, 'Linked Banks', '4 banks connected', onTap: () {
        ...
        SwitchListTile(
          ...
          value: _biometric,
          onChanged: (v) async { await _set('biometric_enabled', v); },
          activeColor: Colors.green,
        ),
        _Tile(Icons.lock_outline, 'Change PIN', 'Update your security PIN', onTap: () {
          showDialog(context: context, builder: (_) {
            final ctrl = TextEditingController();
            return AlertDialog(title: const Text('Change PIN'), content: TextField(controller: ctrl, keyboardType: TextInputType.number, obscureText: true, decoration: const InputDecoration(labelText: 'New PIN', hintText: '4 digits')),
              actions: [
                TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
                FilledButton(onPressed: () async {
                  if (ctrl.text.length == 4) {
                    await _set('app_pin', ctrl.text);
        ...
        _Tile(Icons.timer, 'Auto-Lock', '${_autoLock}s of inactivity', onTap: () {
          showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Auto-Lock Timer'),
            content: Column(mainAxisSize: MainAxisSize.min, children: [15, 30, 60, 120].map((s) => RadioListTile(
              title: Text('$s seconds'), value: s, groupValue: _autoLock,
              onChanged: (v) async { await _set('auto_lock', v); if (mounted) Navigator.pop(context); },
            )).toList()),
          ));
        }),
        ...
        _Tile(Icons.language, 'Language', _language, onTap: () {
          showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Select Language'),
            content: Column(mainAxisSize: MainAxisSize.min, children: ['English', 'አማርኛ (Amharic)', 'Afan Oromo'].map((l) => RadioListTile(
              title: Text(l), value: l, groupValue: _language,
              onChanged: (v) async { await _set('language', v); if (mounted) Navigator.pop(context); },
            )).toList()),
          ));
        }),
        ...
        _Tile(Icons.info_outline, 'Version', '1.0.0 (Redesign)'),
          ...
          child: OutlinedButton.icon(
            onPressed: () async {
              final ok = await showDialog<bool>(context: context,
                builder: (_) => AlertDialog(title: const Text('Reset App'), content: const Text('This will clear all data including PIN, wallet, and settings. Continue?'),
...
class _Tile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return ListTile(leading: Icon(icon), title: Text(title), subtitle: Text(subtitle), trailing: onTap != null ? const Icon(Icons.chevron_right) : null, onTap: onTap);
  }
}
